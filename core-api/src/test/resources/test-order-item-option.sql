...
-- order_item_option 데이터 삽입
INSERT INTO order_item_option (order_id, item_option_id, code_id, group_id, count, sales_price, created_at, modified_at)
VALUES
    (@order_id1, (SELECT id FROM item_option WHERE name = 'Item Option1'), '010', '100', 2, 1100, NOW(), NOW()),
    (@order_id1, (SELECT id FROM item_option WHERE name = 'Item Option2'), '010', '100', 1, 2100, NOW(), NOW()),
    (@order_id2, (SELECT id FROM item_option WHERE name = 'Item Option3'), '020', '100', 3, 3300, NOW(), NOW()),
    (@order_id3, (SELECT id FROM item_option WHERE name = 'Item Option1'), '030', '100', 2, 1100, NOW(), NOW()),
    (@order_id2, (SELECT id FROM item_option WHERE name = 'Item Option1'), '010', '100', 1, 1200, NOW(), NOW()),
    (@order_id3, (SELECT id FROM item_option WHERE name = 'Item Option2'), '010', '100', 2, 2200, NOW(), NOW());